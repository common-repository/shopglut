			...
			if (isset($this->field['options'])) {
				...
				if ((is_array($options) && !empty($options)) || (!empty($args['chosen']) && !empty($args['ajax']))) {
					...
					if (!empty($args['chosen']) && !empty($args['multiple'])) {
						...
						echo '<select name="' . esc_attr($field_name) . '" class="agl-hide-select hidden"' . ($multiple_attr) . ($field_attr) .
							'>';
					...
					if (!empty($args['active_device']) && count($args['device_type']) === 3) {
						...
						foreach ($args['device_type'] as $device_type) {
							...
							echo '<select class="active-device" id="' . esc_attr($select_id) . '" name="' . esc_attr($this->field_name('[' . $select_id . ']')) . '" data-depend-id="' . $args['id'] . '-' . $device_type . '">';
